...
export default function ApartmentExpenseTracker() {
  ...
  const [activeUser, setActiveUser] = useState("Nihal");
  ...
  const [form, setForm] = useState({ date: today(), category: "Grocery", description: "", amount: "", paidBy: "Nihal", split: "50/50", receipt: "" });
  ...
  const cardClass = dark ? "rounded-3xl border border-slate-800 bg-slate-900 p-5" : "rounded-3xl border border-white/80 bg-white/90 p-5 shadow-sm";
  const inputClass = dark ? "rounded-2xl border border-slate-700 bg-slate-950 px-3 py-2 text-white" : "rounded-2xl border border-slate-200 bg-white px-3 py-2";

  return (
    <div className={pageClass}>
      <div className="mx-auto max-w-6xl px-4 py-5">
        <header className="mb-5 rounded-[2rem] bg-gradient-to-br from-indigo-600 via-violet-600 to-teal-500 p-6 text-white shadow-xl">
          <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-sm text-white/80">411 Duplex - Yonge Eglinton</p>
              <h1 className="text-3xl font-bold">Nihal & Shreya’s Budget Tracker</h1>
            ...
            <div className="flex flex-wrap gap-2">
              <select value={activeUser} onChange={(e) => { setActiveUser(e.target.value); setForm((f) => ({ ...f, paidBy: e.target.value })); }} className="rounded-2xl border border-white/40 bg-white/10 px-4 py-2 text-white">
                {people.map((p) => <option key={p} className="text-slate-900">{p}</option>)}
              </select>
              <button onClick={() => setDark(!dark)} className="rounded-2xl border border-white/40 bg-white/10 px-4 py-2 text-white">{dark ? "Light" : "Dark"}</button>
              <button onClick={exportCsv} className="rounded-2xl bg-white px-4 py-2 font-semibold text-indigo-700">CSV / Sheets Backup</button>
            </div>
          </div>
        </header>
        ...
        <section className="mb-5 grid gap-3 md:grid-cols-4">
          ...
          <div className="rounded-3xl bg-gradient-to-br from-indigo-500 to-violet-500 p-5 text-white"><p className="text-sm text-white/80">Monthly Total</p><h2 className="mt-3 text-2xl font-bold">{money(totals.total)}</h2><p className="text-sm text-white/80">{monthExpenses.length} expenses</p></div>
          <div className="rounded-3xl bg-gradient-to-br from-teal-500 to-emerald-500 p-5 text-white"><p className="text-sm text-white/80">Budget</p><input type="number" value={budget} onChange={(e) => setBudget(Number(e.target.value))} className="mt-3 w-full rounded-2xl border border-white/40 bg-white/15 px-3 py-2 text-xl font-bold text-white" /><div className="mt-3 h-2 rounded-full bg-white/25"><div className="h-2 rounded-full bg-white" style={{ width: `${budgetUsed}%` }} /></div></div>
          ...
          <section className="space-y-5">
            <div className={cardClass}>
              ...
              <form onSubmit={saveExpense} className="space-y-3">
                <div className="grid grid-cols-2 gap-3"><input type="date" value={form.date} onChange={(e) => setForm({ ...form, date: e.target.value })} className={inputClass} /><input type="number" min="0" step="0.01" placeholder="Amount" value={form.amount} onChange={(e) => setForm({ ...form, amount: e.target.value })} className={inputClass} /></div>
                ...
                <button type="submit" className="w-full rounded-2xl bg-gradient-to-r from-indigo-600 to-teal-500 px-4 py-3 font-semibold text-white">{editingId ? "Update Expense" : "Add Expense"}</button>
            ...
            <div className={cardClass}>
              ...
              <div className="space-y-2">
                {recurring.map((bill) => (
                  <div key={bill.id} className="grid grid-cols-2 gap-2">
              ...
              <button onClick={addRecurringBills} className="mt-3 w-full rounded-2xl bg-indigo-600 px-4 py-3 font-semibold text-white">Add bills to this month</button>
            </div>
          </section>
